...
public class JobDemo {
    ...
    private static CoordinatorRegistryCenter createRegistryCenter() {
        // ZookeeperConfiguration构造方法两个参数，serverLists(连接Zookeeper服务器的列表，
        // 包括IP地址和端口号，，多个地址用逗号分隔)和namespace(命名空间)
        CoordinatorRegistryCenter registryCenter = new ZookeeperRegistryCenter(
                new ZookeeperConfiguration("192.168.147.128:2181", "elastic-job-demo"));
        registryCenter.init();
    ...
    private static LiteJobConfiguration createJobConfiguration() {
        // 创建简单作业配置构建器，三个参数为：jobName(作业名称)，cron（作业启动时间的cron表达式）
        // ，shardingTotalCount(作业分片总数)
        JobCoreConfiguration simpleCoreConfig = 
                JobCoreConfiguration.newBuilder("mySimpleJob", "0/10 * * * * ?", 12)
                .build();
        // 简单作业配置，第二个参数为jobClass
        SimpleJobConfiguration simpleJobConfig = 
                new SimpleJobConfiguration(simpleCoreConfig,
    ...
    private static void setUpDataflowJob(final CoordinatorRegistryCenter registryCenter) {
        ...
        new JobScheduler(registryCenter, LiteJobConfiguration
                .newBuilder(dataflowJobConfiguration).build()).init();
